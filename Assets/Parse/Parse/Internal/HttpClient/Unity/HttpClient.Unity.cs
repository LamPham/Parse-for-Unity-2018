...
namespace Parse.Internal {
  internal class HttpClient : IHttpClient {
    ...
    private static UnityWebRequestAsyncOperation GenerateWWWInstance(string uri, byte[] bytes, Hashtable headerTable) {
            ...
            string method = bytes != null ? "POST" : "GET";
            var _uwr = new UnityWebRequest(uri, method);
            ...
            foreach (DictionaryEntry pair in headerTable)
                _uwr.SetRequestHeader(pair.Key as string, pair.Value as string);
            return _uwr.SendWebRequest();
        }
  }
}
